...
6. **统一 PostingEditor 组件，但区分编辑/只读两种消费形态**：
   ...
   - **只读形态 = 交易详情页**：postings 一旦入账即**不可变**（beancount 不可变原则；后端 `TransactionQueryService.update` 仅更新 `flag/payee/narration/tags/links/meta`，不接受 postings）。详情页用同一 `PostingEditorRow` **只读展示**（含插值行的"自动"徽标），**不**是可编辑消费者。
...
- `postingsCard` 示范"一行手填 + 一行自动"的配平范式。
...
- 现有 `use_transaction_detail.dart` 的 `postingBalances`（前端按 units 求和）保留用于 `isBalanced` 布尔判断，但不再渲染 Δ 数字。
...
- **firela-vlt 后端插值引擎**：`libs/bean/src/core/interpolation/`（`InterpolationService` / `ResidualCalculator` / `MissingValueFinder`）、`TransactionPipeline`（Booking → Interpolation → Physics）
- **相关 ADR**：ADR-0064（Rule Match 仅处理 Income/Expenses 分类）、ADR-0065（入口层账户有效性保证）
- **决策依据**：多专家并行评估 workflow（4 份独立设计 + 4 份对抗审查，confidence 0.95，8/8 一致收敛）
