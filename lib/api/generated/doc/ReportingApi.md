...
[**reportingControllerGetCashFlowTrends**](ReportingApi.md#reportingcontrollergetcashflowtrends) | **GET** /api/v1/{region}/reporting/cash-flow/trends | Get cash-flow trends
...
# **reportingControllerGetCashFlowTrends**
> CashFlowTrendsResponseDto reportingControllerGetCashFlowTrends(region, period, granularity)

Get cash-flow trends

 Monthly income / expense / netSavings over a fixed N-month window (current month + N−1 prior). Missing months are zero-filled (flow metric).  **Parameters:** - `period`: 1m | 3m | 6m | 1y (default 6m) - `granularity`: accepted for API symmetry; v1 returns month buckets     
...
final String granularity = granularity_example; // String | Data granularity (accepted for API symmetry; v1 returns month buckets)

try {
    final response = api.reportingControllerGetCashFlowTrends(region, period, granularity);
    print(response);
} catch on DioException (e) {
    print('Exception when calling ReportingApi->reportingControllerGetCashFlowTrends: $e\n');
}
```
 ...
 **granularity** | **String**| Data granularity (accepted for API symmetry; v1 returns month buckets) | [optional] [default to 'month']

### Return type

[**CashFlowTrendsResponseDto**](CashFlowTrendsResponseDto.md)
