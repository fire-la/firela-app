import 'package:test/test.dart';
import 'package:firela_api/firela_api.dart';

// tests for RecurringRuleWithStatsResponseDto
void main() {
  final instance = RecurringRuleWithStatsResponseDtoBuilder();
  // TODO add properties to the builder and call build()

  group(RecurringRuleWithStatsResponseDto, () {
    // Rule ID
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // User ID
    // String userId
    test('to test the property `userId`', () async {
      // TODO
    });

    // Rule name
    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // Icon emoji
    // JsonObject icon
    test('to test the property `icon`', () async {
      // TODO
    });

    // Recurring frequency
    // String frequency
    test('to test the property `frequency`', () async {
      // TODO
    });

    // Expected amount
    // num expectedAmount
    test('to test the property `expectedAmount`', () async {
      // TODO
    });

    // Expected day of month
    // JsonObject expectedDay
    test('to test the property `expectedDay`', () async {
      // TODO
    });

    // Custom interval in days
    // JsonObject customIntervalDays
    test('to test the property `customIntervalDays`', () async {
      // TODO
    });

    // Currency code
    // String currency
    test('to test the property `currency`', () async {
      // TODO
    });

    // Payee matching pattern
    // JsonObject matchPayeePattern
    test('to test the property `matchPayeePattern`', () async {
      // TODO
    });

    // Amount tolerance percentage
    // num matchAmountTolerance
    test('to test the property `matchAmountTolerance`', () async {
      // TODO
    });

    // Default expense account
    // JsonObject defaultExpenseAccount
    test('to test the property `defaultExpenseAccount`', () async {
      // TODO
    });

    // Default payment account
    // JsonObject defaultPaymentAccount
    test('to test the property `defaultPaymentAccount`', () async {
      // TODO
    });

    // Default payee
    // JsonObject defaultPayee
    test('to test the property `defaultPayee`', () async {
      // TODO
    });

    // Whether rule is active
    // bool isActive
    test('to test the property `isActive`', () async {
      // TODO
    });

    // Rule start date (YYYY-MM-DD)
    // String startDate
    test('to test the property `startDate`', () async {
      // TODO
    });

    // Rule end date (YYYY-MM-DD)
    // JsonObject endDate
    test('to test the property `endDate`', () async {
      // TODO
    });

    // Auto-create transaction on expected date
    // bool autoCreate
    test('to test the property `autoCreate`', () async {
      // TODO
    });

    // Last matched occurrence date (YYYY-MM-DD)
    // JsonObject lastOccurrence
    test('to test the property `lastOccurrence`', () async {
      // TODO
    });

    // Total matched transactions count
    // num totalCount
    test('to test the property `totalCount`', () async {
      // TODO
    });

    // Created at timestamp
    // DateTime createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // Updated at timestamp
    // DateTime updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // Number of pending expected transactions
    // num pendingCount
    test('to test the property `pendingCount`', () async {
      // TODO
    });

    // Number of overdue expected transactions
    // num overdueCount
    test('to test the property `overdueCount`', () async {
      // TODO
    });

    // Next expected date (YYYY-MM-DD)
    // JsonObject nextExpectedDate
    test('to test the property `nextExpectedDate`', () async {
      // TODO
    });

    // Total amount of all matched transactions
    // num totalAmount
    test('to test the property `totalAmount`', () async {
      // TODO
    });

    // Average amount per transaction
    // num averageAmount
    test('to test the property `averageAmount`', () async {
      // TODO
    });

    // Number of matched transactions
    // num transactionCount
    test('to test the property `transactionCount`', () async {
      // TODO
    });

    // First matched transaction date (YYYY-MM-DD)
    // JsonObject firstDate
    test('to test the property `firstDate`', () async {
      // TODO
    });

    // Last matched transaction date (YYYY-MM-DD)
    // JsonObject lastDate
    test('to test the property `lastDate`', () async {
      // TODO
    });

    // Amount variance (standard deviation squared)
    // num variance
    test('to test the property `variance`', () async {
      // TODO
    });

    // Number of upcoming expected transactions
    // num upcomingCount
    test('to test the property `upcomingCount`', () async {
      // TODO
    });

  });
}
